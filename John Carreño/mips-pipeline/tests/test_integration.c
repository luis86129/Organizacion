 ...
 *
 * Ademas prueba el camino de archivos .bin (escribir, recargar, correr)
...
int main(void)
{
    ...
    /* ---------------- camino de archivos .bin ---------------- */
    pr = prog_get(7);
    CHECK(mem_dump_words(pr.code, pr.n_code, "build/tmp_imem.bin") == 0,
          "se escribe el .bin de instrucciones");
    CHECK(mem_dump_words(pr.data, pr.n_data, "build/tmp_dmem.bin") == 0,
          "se escribe el .bin de datos");

    mem_init_imem(&im);
    mem_init_dmem(&dm);
    regfile_init(&rf);
    CHECK(mem_load_imem(&im, "build/tmp_imem.bin") == 0, "se recarga imem.bin");
    CHECK(mem_load_dmem(&dm, "build/tmp_dmem.bin") == 0, "se recarga dmem.bin");
    CHECK_EQ(im.n_words, pr.n_code, "imem.bin tiene todas las instrucciones");
    for (i = 0; i < pr.n_code; i++) {
        CHECK_EQ(im.w[i], pr.code[i], "la instruccion sobrevivio al .bin");
    }
    pipeline_init(&p, 0u);
    rc = pipeline_run(&p, &im, &dm, &rf, MAX_CYCLES);
    CHECK(rc == 0, "el programa cargado desde .bin corre igual");
